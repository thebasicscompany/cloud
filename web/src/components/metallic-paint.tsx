...
/* React-Bits "MetallicPaint" — JS + CSS variant, ported to TSX. The shader
 * code is verbatim from the React-Bits source; only the prop types and a few
 * ref annotations are TypeScript-flavored. We keep this loose-typed because
 * the underlying WebGL plumbing pre-dates this file and pinning every uniform
 * by name fights the runtime rather than helping it. */
...
export default function MetallicPaint({
  ...
  const initGL = useCallback(() => {
    ...
    const gl: any = canvas.getContext("webgl2", { antialias: true, alpha: true });
    if (!gl) return false;
  ...
  return <canvas ref={canvasRef} className={className ?? "paint-container"} />;
}
