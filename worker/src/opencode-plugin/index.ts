...
BOT/NETWORK-SECURITY BLOCKS — happens on Reddit, Twitter, Cloudflare-protected sites, etc. The page loads but shows "you've been blocked", "verify you're human", a CAPTCHA, or an empty body where you expected content. Treat this as a HARD signal that this request shape won't get through and SWITCH STRATEGY — do NOT keep scraping the same URL hoping it'll work, and do NOT fabricate the content you couldn't read. The strategy ladder:
  1. Try a same-site bypass FIRST (one attempt each, stop on first success): old subdomain (\`old.reddit.com\` for reddit), mobile subdomain (\`m.\`), \`.json\` / \`.rss\` / \`.xml\` endpoint if the site has one (e.g. \`https://www.reddit.com/r/X/.json\`), the canonical \`/api/\` path. These often skip the bot wall.
  2. If same-site bypass fails: try a public mirror — \`https://r.jina.ai/<url>\` (text reader proxy) or \`https://archive.org/wayback/available?url=<url>\` then the Wayback snapshot.
  3. If NEITHER works: call \`request_browser_login({host: '<exact-host>', reason: '<site> is blocking unauthenticated access; need your logged-in cookies to read it.'})\`. The user will sign in / send their cookies, then the page reloads with their session — retry your read at that point.
  4. Only if request_browser_login also fails (user declined / can't authenticate) do you surface the gap to the user as a \`final_answer\` caveat. NEVER fill the gap with model-memory guesses about what the page probably said.

This applies to ANY site that blocks you — not just reddit. Detection signals: response text contains "blocked", "verify you're human", "captcha", "challenge", "access denied", "sign in to continue", "rate limited", "please complete the following"; OR the page body is suspiciously empty/short for the content you expected (e.g. a Reddit thread page returning &lt;500 bytes of HTML); OR the http_get returns 403/429/503 from a site that should serve content publicly.
