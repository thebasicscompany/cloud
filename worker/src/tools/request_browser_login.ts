...
export const request_browser_login = defineTool({
  ...
  description:
    "Call this when a task needs the user signed in to a website (a browser login / cookies) that this run is NOT already authenticated for — e.g. a personal feed, inbox, dashboard, or account page on a site that isn't in <browser_sites>. It surfaces a one-click 'Sign in to <host>' prompt to the user (on the run and on Home). Pass the registrable host like 'youtube.com'. Then stop and tell the user to connect it and re-run. NEVER try to enter a password yourself.",
  params: z.object({
    host: z.string().min(1).describe("The site host that needs a login, e.g. 'youtube.com' or 'linkedin.com'."),
