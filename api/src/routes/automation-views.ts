...
/**
 * Automations read model — backed by the live `automations` table and
 ...
 * empties/zeros rather than fabricated numbers.
 *
 * Ported VERBATIM from the web data lib `web/src/lib/automations-data.ts`, but
 * scoped to the VERIFIED workspace JWT (`c.var.workspace.workspace_id`) instead
 * of a service-role admin client + a hardcoded PRIMARY_WORKSPACE_ID. Each token
 * only ever reads its own workspace's automations.
 *
 *   GET /v1/automation-views      → { automations: CloudAutomationSummary[] }
 *   GET /v1/automation-views/:id  → { detail: { automation, runs } | null }
 */

type Vars = { requestId: string; workspace: WorkspaceToken }
export const automationViewsRoute = new Hono<{ Variables: Vars }>()
...
// ---- Mapping types (mirror @/types/cloud-automation, kept local so the route
// builds plain objects and never imports the web type module). ----

type CloudAutomationStatus = 'draft' | 'active' | 'paused' | 'archived'
type CloudTriggerStatus = 'registered' | 'paused' | 'manual_only' | 'needs_credentials'
type CloudApprovalMode = 'manual_review' | 'risk_based' | 'trusted_autonomous'
type CloudAutomationRunStatus =
  | 'pending'
  | 'booting'
  | 'running'
  | 'paused_by_user'
...
type CloudAutomationTrigger =
  | { id: string; type: 'manual'; status: CloudTriggerStatus }
  | {
      id: string
      type: 'schedule'
      cron: string
...
function mapApprovalPolicy(raw: unknown): CloudApprovalPolicy {
  ...
  const mode = (['manual_review', 'risk_based', 'trusted_autonomous'] as const).includes(
...
// Local structural alias so the helpers below read like the web lib. The actual
// object shape is what `c.json()` serializes — we never import the web type.
type CloudAutomationRun = {
  id: string
  automationId: string
...
function mapRun(r: RunRow, automationName: string, ws: string): CloudAutomationRun {
  return {
    id: r.id,
...
function mapAutomation(row: AutomationRow) {
  ...
  return {
    ...
    goal: row.goal ?? '',
    source: 'seeded_cloud' as const,
    runTarget: (row.run_target === 'local' ? 'local' : 'cloud') as 'cloud' | 'local',
    status: (['draft', 'active', 'paused', 'archived'] as const).includes(
    ...
    triggers,
    outputs: [] as Array<{
      channel: 'desktop' | 'sms' | 'email' | 'artifact'
      target: string
      when: 'on_complete' | 'on_failure'
    }>,
    requiredCredentials: requiredCredentialsFrom(triggers),
    checkModules: [] as string[],
    approvalPolicy: mapApprovalPolicy(row.approval_policy),
    trustGrants: [] as unknown[],
...
automationViewsRoute.get('/', requireWorkspaceJwt, async (c) => {
  ...
  if (automations.length === 0) return c.json({ automations: [] })
  ...
  const byAutomation = new Map<
    string,
    { runsLast7d: number; total: number; completed: number; last?: { id: string; status: string; startedAt: string } }
  >()
  ...
  const result = automations.map((row) => {
    ...
    return {
      ...
      lastRun: agg?.last
        ? { id: agg.last.id, status: lastRunStatus ?? 'completed', startedAt: agg.last.startedAt }
        : undefined,
...
automationViewsRoute.get('/:id', requireWorkspaceJwt, async (c) => {
  const ws = c.var.workspace.workspace_id
  ...
  const { data: row } = await supabase
    ...
    .eq('workspace_id', ws)
    .maybeSingle()
  if (!row) return c.json({ detail: null })
  ...
  const { data: runRows } = await supabase
    ...
    .order('created_at', { ascending: false })
    .limit(50)
  const runs = ((runRows ?? []) as RunRow[]).map((r) => mapRun(r, automation.name, ws))
  return c.json({ detail: { automation: { ...automation, lastRunId: runs[0]?.id }, runs } })
})
