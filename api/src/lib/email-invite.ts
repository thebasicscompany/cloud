 ...
 *
 * Ported verbatim from web `web/src/lib/email-invite.ts`, but reads the sender
 * + region from the runtime API config (cloud/api owns the SES creds — the
 * renderer must NOT). Returns `{ ok:false }` (never throws) when SES_FROM_EMAIL
 * is unset so the invite endpoint can still return the accept link.
 */
let _ses: SESv2Client | null = null
function ses(): SESv2Client {
  if (!_ses) _ses = new SESv2Client({ region: getConfig().AWS_REGION ?? 'us-east-1' })
  return _ses
}
...
export async function sendInviteEmail(input: {
  ...
  const from = getConfig().SES_FROM_EMAIL
